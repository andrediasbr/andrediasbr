# Badges de Certificações

Este diretório contém todos os badges baixados das certificações do Credly.

## Total: 58 badges

### 🌟 MVP
- `mvp-alumni.png` - Microsoft MVP Alumni

### ☁️ Microsoft Azure
- `azure-solutions-architect.png` - Azure Solutions Architect Expert
- `azure-devops-engineer.png` - Azure DevOps Engineer Expert
- `azure-developer.png` - Azure Developer Associate
- `azure-administrator.png` - Azure Administrator Associate
- `azure-security.png` - Azure Security Engineer Associate
- `azure-data-fundamentals.png` - Azure Data Fundamentals
- `azure-fundamentals.png` - Azure Fundamentals
- `azure-ai-fundamentals.png` - Azure AI Fundamentals
- `azure-ai-transformation-leader.svg` - Microsoft Certified: AI Transformation Leader
- `m365-fundamentals.png` - Microsoft 365 Certified: Fundamentals

### ☁️ Amazon Web Services (AWS)
- `aws-solutions-architect.png` - AWS Solutions Architect Associate
- `aws-sysops.png` - AWS SysOps Administrator Associate
- `aws-cloud-practitioner.png` - AWS Cloud Practitioner
- `aws-well-architected.png` - Well-Architected Proficient

### ☁️ Google Cloud Platform (GCP)
- `gcp-architect.png` - Professional Cloud Architect
- `gcp-associate-engineer.png` - Associate Cloud Engineer

### 🔧 GitHub
- `github-administration.png` - GitHub Administration
- `github-advanced-security.png` - GitHub Advanced Security
- `github-actions.png` - GitHub Actions
- `github-copilot.png` - GitHub Copilot
- `github-foundations.png` - GitHub Foundations
- `github-migrations.png` - GitHub Migrations Partner Delivery Credential
- `github-azdo-migrations.png` - AzureDevOps to GitHub Migrations Delivery Credential
- `github-ghas-delivery.png` - GitHub Advanced Security Partner Delivery Credential
- `github-copilot-delivery.png` - GitHub Copilot: Core Skills & Application Partner Delivery Credential
- `github-tech-sales.png` - GitHub Tech Sales Professional
- `github-sales-professional.png` - GitHub Sales Professional
- `github-sales-professional-fy26.png` - FY26 GitHub Sales Professional
- `github-platform-sales.png` - FY26 GitHub Platform Sales Badge
- `github-security-sales.png` - FY26 GitHub Advanced Security Sales Badge
- `github-copilot-sales.png` - FY26 GitHub Copilot Sales Badge
- `github-revenue-motions.png` - FY26 GitHub Revenue Motions Sales Badge
- `github-agentic-ai-developer.png` - GitHub Certified: Agentic AI Developer
- `github-copilot-enterprise-delivery.png` - GitHub Copilot: Deploying and Scaling AI in the Enterprise Delivery Credential
- `github-actions-cicd-delivery.png` - Modernizing Enterprise CI/CD with GitHub Actions Delivery Credential

### 🔧 HashiCorp
- `terraform.png` - Terraform Associate (002)

### 📚 Scrum.org
- `sps-scrum.png` - Scaled Professional Scrum (SPS)
- `psm.png` - Professional Scrum Master I (PSM I)
- `psd-scrum.png` - Professional Scrum Developer I (PSD I)

### 📚 Certiprof
- `scrum-foundation.png` - Scrum Foundation Professional (SFPC)
- `devops-essentials.png` - DevOps Essentials Professional (DEPC)
- `lifelong-learning.png` - Lifelong Learning

### 🎓 Microsoft Certified Trainer
- `mct-2017-2018.png` - MCT 2017-2018
- `mct-2020-2021.png` - MCT 2020-2021
- `mct-2021-2022.png` - MCT 2021-2022
- `mct-2022-2023.png` - MCT 2022-2023
- `mct-2026.png` - MCT 2026

### 📝 Microsoft Exams
- `az-400.png` - AZ-400: Designing and Implementing Microsoft DevOps Solutions
- `exam-532.png` - Exam 532: Developing Microsoft Azure Solutions
- `exam-533.png` - Exam 533: Implementing Microsoft Azure Infrastructure Solutions
- `exam-534.png` - Exam 534: Architecting Microsoft Azure Solutions
- `exam-535.png` - Exam 535: Architecting Microsoft Azure Solutions
- `exam-496.png` - Exam 496: Administering Visual Studio Team Foundation Server
- `exam-497.png` - Exam 497: Software Testing with Visual Studio
- `exam-498.png` - Exam 498: Delivering Continuous Value with Visual Studio ALM

### 🏅 Microsoft Legacy Certifications
- `mcsa-cloud.png` - MCSA: Cloud Platform 2017
- `mcse-cloud-2017.png` - MCSE: Cloud Platform and Infrastructure 2017
- `mcse-cloud-2018.png` - MCSE: Cloud Platform and Infrastructure 2018

### 🌱 Sustentabilidade
- `green-software.png` - LFC131: Green Software for Practitioners (Linux Foundation)

## Como usar

Para incluir um badge no README, use:
```markdown
<img src="assets/badges/nome-do-arquivo.png" alt="Descrição" width="100"/>
```

## Organização

Você pode excluir os badges que não deseja exibir no perfil principal. 
Estes arquivos servem como referência completa de todas as suas certificações.
